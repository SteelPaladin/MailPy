...
def Logon(user, passw):
    try:
        mail = imaplib.IMAP4_SSL('smtp.gmail.com','993') #993, 587, 465
        #Running without a defined port appears to return quicker. Test this on working network.
        mail.ehlo()
        mail.starttls()
...
class LoginWindow:
    def __init__(self, root):
        ...
        user=(self.e.get())
        passw=(self.e1.get())
        self.b1=Button(root, text="ENTER", command=lambda:Logon((user),(passw))).pack(side=RIGHT)
        global user, passw
